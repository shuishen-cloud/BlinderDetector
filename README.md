...
| ② 开发者面板 `#devPanel` | **契约验证用的测试件，默认 `hidden`** | 帧源 ①②③、其余路由、请求日志、会话统计、`/v1/health` 明细 |

第二层的入口有两个：地址栏 `?dev=1`，或右上角 ⚙；选择记在 `localStorage`，
`?dev=0` 收起。**「藏起来」是 `hidden` 而不是删掉** —— `app.js` / `map.js`
一律按 id 取 DOM，元素真被删掉会当场报错；而帧源、其余路由这些验证契约的
东西，随时要能翻出来。
...
**开发者面板里的测试件**（帧源、其余路由 —— 交付形态里不该出现的东西）：
...
web/
  index.html            ★ 前端结构（零构建，无打包步骤）
  app.css               样式（深色主题的全部取值）
  app.js                前端逻辑（WS 渲染 / 视图开关 / 帧源）
